...
// Streaming helper to accumulate chunks & parse when done
window.parseQuizFromStream = async function(streamPromise, topic = '') {
    let fullText = "";

    const reader = (await streamPromise).body.getReader();
    ...
    console.log("📥 Final Streamed Text:", fullText);
    return parseQuiz(fullText, topic);
};
